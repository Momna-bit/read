...
-- TASK 4 (FULL REBUILD): Baseline forecasting dataset, end-to-end, Texas
-- Combines: calendar, active customer count, corrected past-due count,
-- corrected containment rate, abandon rate, avg talk time, combined transfers,
-- Alberta data-availability flag

WITH Calendar AS (
    SELECT 
        CAST(CalendarDate AS DATE) AS CallDay,
        DATENAME(WEEKDAY, CalendarDate) AS Weekday,
        IsHoliday  -- CONFIRM actual column name on vw_calendarWH before running
    FROM vw_calendarWH
    WHERE CalendarDate >= '2022-07-01'
),
