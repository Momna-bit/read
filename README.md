...
-- STEP 2: Check IVRQueue's columns
SELECT COLUMN_NAME, DATA_TYPE
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_NAME = 'IVRQueue'
ORDER BY ORDINAL_POSITION;


-- STEP 3: Check Care_IVRKPISummary's columns instead
SELECT COLUMN_NAME, DATA_TYPE
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_NAME = 'Care_IVRKPISummary'
ORDER BY ORDINAL_POSITION;


-- Backup: broader search in case the real table has a different name
SELECT TABLE_NAME
FROM INFORMATION_SCHEMA.TABLES
WHERE TABLE_NAME LIKE '%KPI%'
   OR TABLE_NAME LIKE '%Volume%'
