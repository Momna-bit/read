...
-- (cutoff date calculated once into a variable, since SQL Server won't
-- allow a subquery directly inside GROUP BY)
-- =====================================================================
DECLARE @CutoffDate DATE;
SELECT @CutoffDate = DATEADD(MONTH, -3, MAX(CallDay)) FROM #Actual;

SELECT
    CASE WHEN c.CallDay >= @CutoffDate THEN 'Most Recent 3 Months' ELSE 'Older 9 Months' END AS Window_,
...
GROUP BY CASE WHEN c.CallDay >= @CutoffDate THEN 'Most Recent 3 Months' ELSE 'Older 9 Months' END
